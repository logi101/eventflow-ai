...
- AI chat suggest + confirm pattern (never autonomous writes)
- Greedy seating algorithm chosen over CSP library for v1 (sufficient for <500 participants)
- Dexie.js 4.x for IndexedDB (ecosystem standard)
- 24-hour TTL for cached data (Safari ITP compliance)
- Append-only audit log with RLS policies (INSERT/SELECT only)
- 8 simulation validators with deterministic IDs
- Contingency uses same suggest+confirm pattern as AI actions

**Issues Resolved:**

- uuid_generate_v4() vs gen_random_uuid() (PostgreSQL 13+ compatibility)
- Supabase CLI authentication in non-TTY environments
- Migration history sync between local and remote
- VIP sorting across filtered lists
- Offline-first check-in with optimistic UI updates
