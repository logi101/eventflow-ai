...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (action === "update_password") {
      ...
      // Cannot change super_admin password (unless you are the same user)
      const { data: targetProfile } = await adminClient
        .from("user_profiles")
        .select("role")
        .eq("id", user_id)
      ...
      if (targetProfile?.role === "super_admin" && user_id !== caller.id) {
        return new Response(
          JSON.stringify({ error: "Cannot change another super_admin's password" }),
          { status: 403, headers: { ...corsHeaders, "Content-Type": "application/json" } }
        );
      }
